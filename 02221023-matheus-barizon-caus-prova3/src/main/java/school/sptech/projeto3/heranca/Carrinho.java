...
public class Carrinho {
    ...
    /**
     * Encontra e retorna um {@link Produto} com o id especificado Caso o
     * produto com id desejado não exista retorne null
     *
    ...
    public List<String> findNomesRepetidos() {
        ...
        for (Integer produto = 0; produto < produtos.size(); produto++) {
            for (Integer repetido = produto; repetido < produtos.size(); repetido++) {
                if (produtos.get(produto).getNome().equals(produtos.get(repetido).getNome()) && repetido != produto) {
                    nomesRepetidos.add(produtos.get(produto).getNome());
                }
            }
        }
